...
2. 請根據第一題的結果，計算出各年級中，不同性別的人數，以及人數在各年級中所佔比例，並將結果取名為`A_A`。(5分)
...
3. 請根據第一題的結果，計算出各年級的總人數，以及各年級中的GPA平均和標準差，並以GPA平均降冪排列，將結果取名為`T_T`。(5分)
...
4. 我要打包！請撰寫一個函數，此函數的 argument 為

- `df`: 一個 `tibble` (必要)
- `grades`: 年級 (預設為全部年級)
- `values`: 數值 (預設為全部數值)  
其結果會根據條件回傳所選擇年級的 q_self 平均、q_teacher 平均、以及 GPA 平均等數值。(5分)

```{r message=FALSE}
compute_mean <- function(df, grades, values) {
  # Write your code here
}
